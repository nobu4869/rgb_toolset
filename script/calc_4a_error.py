...
#    l_RGB = np.matrix( ((sRGB/255))**(1/2.2))
#    l_RGB = np.matrix ((sRGB/255) )
    ...
    print("X+Y+Z :ave:" , np.average(sum_xyz) , " error:" , (np.max(sum_xyz)-np.min(sum_xyz))/np.min(sum_xyz)*100.0 , "std:" , np.std(sum_xyz))
    ...
    print("x: ave:" , np.average(xy_x) , " error:" , (np.max(xy_x)-np.min(xy_x))/np.min(xy_x)*100.0 , "std:" , np.std(xy_x))
    print("y: ave:" , np.average(xy_y) , " error:" , (np.max(xy_y)-np.min(xy_y))/np.min(xy_y)*100.0 , "std:" , np.std(xy_y))
    return (xy_x,xy_y)
